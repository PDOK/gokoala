	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				...
				dtSupport: true,
				cqlConfig: config.CQL{Enable: types.PtrTo(true), EnableAdvancedComparisonOperators: types.PtrTo(true)},
			},
			wantLimit:     1,
			wantOutputCrs: 100000,
			wantInputCrs:  100000,
...
func generateRandomString(length int) string {
	const charset = "abc"
	seed := rand.New(rand.NewPCG(rand.Uint64(), rand.Uint64())) //nolint:gosec  // good enough for testing
